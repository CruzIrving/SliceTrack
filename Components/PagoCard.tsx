...
const PagoCard = ({ onValidChange }: Props) => {
  ...
  const metodoActual = useMemo(
    () =>
      pagos.find((p) => p.id === selectedPaymentId) ??
      (pagos.length > 0 ? pagos[pagos.length - 1] : null),
  ...
  const canAdd =
    isCardNumberValid && isCvvValid && expiryIsValid() && isHolderValid;
        ...
        <View style={styles.cardContent}>
          ...
          <TouchableOpacity
            onPress={() => {
              ...
              setShowNewForm(pagos.length === 0);
            }}
            style={styles.buttonRight}
          >
            <Text style={styles.changeBtn}>
              {metodoActual ? "Cambiar" : "Agregar"}
            </Text>
          </TouchableOpacity>
        </View>
              ...
              {pagos.length > 0 && !showNewForm ? (
                ...
                <ScrollView>
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      setShowNewForm(false);
                      if (pagos.length === 0) setModalVisible(false);
                    }}
                    style={{ marginTop: 10 }}
                  >
...
const styles = StyleSheet.create({
  ...
  label: { fontSize: 18, fontWeight: "900", color: "#000", marginBottom: 6 },
